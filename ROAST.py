...
class ExpertInstanceGenerator(ROASTDriver):
    ...
    # The methods below are unchanged as their logic is independent of the prompt text.
    ...
    def _find_fuzzy_positions(self, context: str, span: str, threshold: int = 85) -> Tuple[str, int, int]:
        """
    ...
    def _parse_response(self, response_text: str, context: str) -> Optional[List[Tuple[str, float, int, int]]]:
        ...
        if parsed_list is None:
            try:
                ...
                logging.warning(f"Failed to parse text as a list with both json and ast: {list_text}")
                return None
        if parsed_list is None:
            return None
        # --- MODIFIED PART ---
        ...
        for item in parsed_list:
            ...
            if start == -1 or end == -1:
                logging.warning(f"Could not find instance '{generated_instance}' in the original context.")
                # Use the new fuzzy finding method
                found_instance, start, end = self._find_fuzzy_positions(context, generated_instance, threshold=60)
                if found_instance:
                    results.append((found_instance, 1.0, start, end))
                else:
                    logging.warning(f"Could not find a confident fuzzy match for '{generated_instance}' in the context.")
            else:
    ...
    def extract(self, context: str, abstract_concept: str, explanation_of_abstract: Optional[str] = None) -> List[Tuple[str, float, int, int]]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logging.warning(f"Initial parsing failed on attempt {attempt + 1}. Raw response: '{raw_response_text}'")
                ...
                logging.warning(f"Self-correction attempt {attempt + 1} also failed to yield a parsable list.")
            except Exception as e:
                logging.error(f"Ollama API call failed on attempt {attempt + 1}/{self.max_retries}: {e}")
        logging.error(f"Failed to get a valid, parsable response after {self.max_retries} attempts and self-correction.")
        return []
...
TEST_CASES = [
    ...
    },
    {
        ...
        "instances": ["Order of the Silver Griffin", "Order of the Obsidian Serpent",
    ...
    },
    {
        ...
        "instances": ["Operation Nightfall", "Operation Sandstorm", "Operation Thunderbolt"]
    },
    {
        "id": 77,
    ...
    },
    {
        ...
        "instances": ["Gryffindor", "Slytherin", "Ravenclaw", "Hufflepuff"]  # Note: 4 instances
    },
    {
        "id": 81,
    ...
    },
    {
        ...
        "instances": ["Weyland-Yutani Corporation", "Tyrell Corporation", "Cyberdyne Systems Corporation"]
    },
    {
        "id": 92,
...
def evaluate_extractor(extractor, test_cases):
    ...
    for test in test_cases:
        ...
        # run extractor
        predictions = [
            span for span, score, start, end in extractor.extract(
                test["context"], test["abstract"],
...
def plot_performance(case_results, overall, model_name):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f"./roast_bench/Per-Test-Case Performance {model_name}.png", dpi=300, bbox_inches="tight")

    # Overall TP/FP/FN bar chart
    plt.figure(figsize=(6, 5))
    plt.bar(["TP", "FP", "FN"], [overall["TP"], overall["FP"], overall["FN"]],
            color=["green", "red", "orange"])
    plt.title(f"Overall Instance-Level Confusion Counts {model_name}")
    plt.ylabel("Count")
    plt.savefig(f"./roast_bench/Overall Instance-Level Confusion Counts {model_name}.png", dpi=300, bbox_inches="tight")
