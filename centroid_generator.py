...
SENSE_CONFIG = {
    ...
    "class_names": [
        "Normal and neutral", "Love and romantic", "War and combat",
        "Fantasy and mythology", "Honor and respect", "Drama and tragedy",
        "City and Crowd", "Mountain and the heights", "Desert and dunes",
        "Sea and tides", "Forest and tress"
    ]
}
...
# =============================================================================
# 2. GENERALIZED CORE FUNCTIONS
# =============================================================================

def create_and_save_centroids(df: pd.DataFrame, embedder: SentenceTransformer, class_id_col: str,
    ...
    for class_id in unique_class_ids:
        class_embeddings = np.vstack(df[df[class_id_col] == class_id]['embedding'].values)
...
def evaluate_with_centroids(test_cases: list, config: dict, embedder: SentenceTransformer, centroids: np.ndarray):
    """
    ...
    test_embeddings = embedder.encode(texts, convert_to_numpy=True, show_progress_bar=True)
    ...
    print(classification_report(true_labels, pred_labels, target_names=class_names, digits=2))
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    assert {'text', 'sense_class_id', 'age_class_id'}.issubset(main_df.columns), "CSV missing required columns"
    ...
    age_centroids, _ = create_and_save_centroids(
        df=df_with_embeddings,  # Reuse dataframe with embeddings
        embedder=main_embedder,
