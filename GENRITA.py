...
class NNDriver(GENRITADriver):
    ...
    def _load_classifier(self, checkpoint_path: str):
        ...
        try:
            ...
            model = RoBERTaMultiTaskClassifier.load_from_checkpoint(
                checkpoint_path=checkpoint_path, map_location=self.device
            )
            model.freeze()
            model.eval()
            return model
    ...
    def _predict_with_probabilities(self, text: str):
        ...
        return (
            torch.softmax(sense_logits, dim=1).squeeze(),
            torch.softmax(age_logits, dim=1).squeeze(),
        )

    def _get_best_allowed_prediction(
        self, probabilities: torch.Tensor, id_to_name_map: dict, allowed_ids: set = None
    ):
        ...
        return {
            "class_name": "No allowed class found",
            "class_id": -1,
            "confidence": 0.0,
        }
...
class LLMDriver(GENRITADriver):
    ...
    def __init__(self, ollama_model_name: str, max_retries: int = 3):
        ...
        if ollama is None:
            raise ImportError(
                "The 'ollama' library is required. Please run 'pip install ollama'."
            )
        ...
        try:
            self.client.list()
            logging.info(
                f"LLM Driver connected to Ollama, using model: {self.model_name}"
            )
        except Exception as e:
            logging.error(
    ...
    def _call_llm(self, prompt: str) -> dict:
        ...
        try:
            ...
            logging.error(
                f"Failed to get a parsable response from LLM after {self.max_retries} attempts."
            )
            return {}
            ...
            logging.error(
                f"A non-retriable API error occurred, stopping immediately: {e}"
            )
            return {}
    ...
    def _parse_llm_response(self, response_text: str) -> dict:
        ...
        if not match:
            logging.warning(
                f"Parser could not find a JSON-like object in the response: {response_text}"
            )
            return {}
        ...
        try:
            ...
            try:
                ...
                logging.warning(
                    f"Both json.loads and ast.literal_eval failed. Error: {e}. Raw string: {json_str}"
                )
                return {}
        ...
        if (
            ...
            and "preds" in parsed_dict
            and isinstance(parsed_dict["preds"], dict)
        ):
        ...
        for key, value in final_dict.items():
            try:
                ...
                logging.warning(
                    f"Could not convert value '{value}' for key '{key}'. Discarding this entry."
                )
                continue
        return cleaned_dict
    ...
    def response_cleaner(self, response: ollama.GenerateResponse):
        ...
        if start_index != -1 and end_index != -1:
            ...
            if (
                ...
                and "preds" in parsed
                and isinstance(parsed["preds"], dict)
            ):
            ...
            if not parsed_dict:
                logging.warning(
                    f"LLM did not return a dictionary-like object. Response: {response_text}"
                )
                return {}
    ...
    def _get_best_prediction_from_llm_output(
        self,
        preds: dict,
        class_map: dict,
        id_to_name_map: dict,
...
TEST_CASES = [
    {
        ...
        "sense_prediction": {"class_name": "Normal and neutral", "class_id": 0},
        "age_prediction": {
    ...
    },
    {
        ...
        "sense_prediction": {"class_name": "Love and romantic", "class_id": 1},
        "age_prediction": {
            "class_name": "neutral and not special age (non-ancient, non technology)",
            "class_id": 1,
        },
    },
    {
        ...
        "sense_prediction": {"class_name": "War and combat", "class_id": 2},
        "age_prediction": {"class_name": "technology modern age", "class_id": 2},
    },
    {
        ...
        "sense_prediction": {"class_name": "Fantasy and mythology", "class_id": 3},
        "age_prediction": {"class_name": "ancient and old age", "class_id": 0},
    },
    {
        ...
        "sense_prediction": {"class_name": "Honor and respect", "class_id": 4},
        "age_prediction": {"class_name": "ancient and old age", "class_id": 0},
    },
    {
        ...
        "sense_prediction": {"class_name": "Drama and tragedy", "class_id": 5},
        "age_prediction": {
            "class_name": "neutral and not special age (non-ancient, non technology)",
            "class_id": 1,
        },
    },
    {
        ...
        "sense_prediction": {"class_name": "City and Crowd", "class_id": 6},
        "age_prediction": {"class_name": "technology modern age", "class_id": 2},
    },
    {
        ...
        "sense_prediction": {"class_name": "Mountain and the heights", "class_id": 7},
        "age_prediction": {
            "class_name": "neutral and not special age (non-ancient, non technology)",
            "class_id": 1,
        },
    },
    {
        ...
        "sense_prediction": {"class_name": "Desert and dunes", "class_id": 8},
        "age_prediction": {"class_name": "ancient and old age", "class_id": 0},
    },
    {
        ...
        "sense_prediction": {"class_name": "Sea and tides", "class_id": 9},
        "age_prediction": {
            "class_name": "neutral and not special age (non-ancient, non technology)",
            "class_id": 1,
        },
    },
    {
        ...
        "sense_prediction": {"class_name": "Forest and tress", "class_id": 10},
        "age_prediction": {
            "class_name": "neutral and not special age (non-ancient, non technology)",
            "class_id": 1,
        },
...
def evaluate_classifier(driver: GENRITADriver, test_cases: list) -> dict:
    ...
    results = {
        ...
        "sense_confusion_matrix": confusion_matrix(
            y_true_sense, y_pred_sense, labels=report_sense_labels
        ),
        "age_confusion_matrix": confusion_matrix(
            y_true_age, y_pred_age, labels=report_age_labels
        ),
        "sense_classification_report": classification_report(
            y_true_sense,
            y_pred_sense,
            labels=report_sense_labels,
            target_names=report_sense_names,
            zero_division=0,
        ),
        "age_classification_report": classification_report(
            y_true_age,
            y_pred_age,
            labels=report_age_labels,
            target_names=report_age_names,
            zero_division=0,
        ),
...
def plot_performance(results: dict, driver_name: str, save=False):
    ...
    sns.heatmap(
        results["sense_confusion_matrix"],
        annot=True,
        fmt="d",
        cmap="Blues",
        ax=axes[0],
        xticklabels=results["sense_class_names"],
        yticklabels=results["sense_class_names"],
    )
    ...
    sns.heatmap(
        results["age_confusion_matrix"],
        annot=True,
        fmt="d",
        cmap="Greens",
        ax=axes[1],
        xticklabels=results["age_class_names"],
        yticklabels=results["age_class_names"],
    )
...
if __name__ == "__main__":
    ...
    try:
        if driver == "nn":
            ...
            driver_name = (
                f"Large Language Model ({LLM_DRIVER_PARAMS['ollama_model_name']})"
            )
